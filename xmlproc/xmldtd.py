...
from xmlproc.xmlutils import *
from xmlproc.xmlapp import *
...
class WFCDTD(DTDConsumer):
    ...
    def reset(self):
        ...
        for name in list(predef_ents.keys()):
    ...
    def dtd_end(self):
        ...
        for elem in list(self.elems.values()):
            self.attrinfo[elem.get_name()] = (
                elem.get_default_attributes(),
    ...
    def new_external_entity(self, ent_name, pubid, sysid, ndata):
        ...
        if ndata != '' and hasattr(self, 'notations'):
            if ndata not in self.notations:
...
class CompleteDTD(WFCDTD):
    ...
    def get_notations(self):
        """Returns the names of all declared notations."""
        return list(self.notations.keys())
    ...
    def new_element_type(self, elem_name, elem_cont):
        ...
        if elem_cont == 'EMPTY':
            elem_cont = ('', [], '')
            self.elems[elem_name] = ElementType(
                elem_name, make_empty_model(), elem_cont
            )
        elif elem_cont == 'ANY':
            elem_cont = None
        ...
        if elem_name in self.attlists:
            for (attr, a_type, a_decl, a_def) in self.attlists[elem_name]:
                self.elems[elem_name].add_attr(
                    attr, a_type, a_decl, a_def, self.parser
                )
...
class ElementType:
    ...
    def get_valid_elements(self, state):
        ...
        try:
            return list(self.content_model[state].keys())
        except KeyError:
            return []
    ...
    def get_default_attributes(self):
        defs = {}
        for attr in list(self.attrhash.values()):
...
class Attribute:
    ...
    def __init__(self, name, attrtype, decl, default, parser):
        ...
        if default and not self.type == 'CDATA':
        ...
        if name == 'xml:space':
            ...
            if len(self.type) != 2:
                ...
                if (
                    self.type[0] == 'default' and self.type[1] == 'preserve'
                ) or (
    ...
    def validate(self, value, parser):
        ...
        if type(self.type) != bytes:
            ...
            self.type == 'ID' or self.type == 'IDREF' or self.type == 'ENTITIY'
        ):
...
class ContentModel:
    ...
    def add_contents(self, builder, loop=0):
        ...
        if isinstance(self.contents[0], type):
            if loop:
                builder.remember_state()
...
def add_transitions(
    ...
    new_trans = (
        {}
    )   # Hash from label to a list of the possible destination states
    ...
    for (over, destlist) in list(new_trans.items()):
        # creating new state
...
def print_states(states, stop=0):
    assert not ('start' in states or 'final' in states)

    for trans_key in list(states.keys()):
...
def make_model(cmhash, content_model, err):
    ...
    if cm in cmhash:
        ...
        content_model = fnda2fda(
            builder.get_automaton(), builder.get_current_state(), err
        )

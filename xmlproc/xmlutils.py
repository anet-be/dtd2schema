...
from xmlproc import xmlapp, errors
...
class EntityParser:
    ...
    def reset(self):
        ...
        self.last_break = 0
        self.block_offset = (
            0  # Offset from start of stream to start of cur block
        )
        self.pos = 0
        self.last_upd_pos = 0
    ...
    def feed(self, new_data):
        ...
        self.data += new_data
        self.data = self.data.replace('\015\012', '\012')
    ...
    def is_external(self):
        ...
        return (
            self.ent_stack != []
            and self.ent_stack[0][0] != self.get_current_sysid()
        )

    # --- Internal methods

    def _push_ent_stack(self, name='None'):
        self.ent_stack.append(
            (
                self.get_current_sysid(),
                self.data,
                self.pos,
                self.line,
                self.last_break,
...
class XMLCommonParser(EntityParser):
    def parse_external_id(self, required=0, sysidreq=1):
        ...
        if self.now_at('SYSTEM'):
            self.skip_ws(1)
            sys_id = self.get_wrapped_match(
                [('"', reg_sysid_quote), ("'", reg_sysid_apo)]
            )
        elif self.now_at('PUBLIC'):
            self.skip_ws(1)
            pub_id = self.get_wrapped_match(
                [('"', reg_pubid_quote), ("'", reg_pubid_apo)]
            )
    ...
    def parse_xml_decl(self, handler=None):
        ...
        textdecl = (
            self.is_external()
        )   # If this is an external entity, then this
        ...
        if (
            self.get_column() != 5
            or self.get_line() != 1
            or (self.ent_stack != [] and not textdecl)
        ):
            if textdecl:
...
def join_sysids_win32(base, url):
    if (
        len(urllib.parse.urlparse(base)[0]) < 2
    ):   # Handles drive identifiers correctly
...
reg_attr_type = re.compile(
    r'CDATA|IDREFS|IDREF|ID|ENTITY|ENTITIES|NMTOKENS|' 'NMTOKEN'
)   # NOTATION support separate
...
reg2code = {
    reg_name.pattern: 3900,
    reg_ver.pattern: 3901,
    reg_enc_name.pattern: 3902,
    reg_std_alone.pattern: 3903,
    reg_comment_content.pattern: 3904,
    reg_hex_digits.pattern: 3905,
    reg_digits.pattern: 3906,
    reg_pe_ref.pattern: 3907,
    reg_attr_type.pattern: 3908,
    reg_attr_def.pattern: 3909,

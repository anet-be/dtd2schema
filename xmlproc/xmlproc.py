...
from xmlproc.dtdparser import *
from xmlproc.xmlutils import *
from xmlproc.xmlapp import *
from xmlproc.xmldtd import *
...
class XMLProcessor(XMLCommonParser):
            ...
            while self.pos < self.datasize:
                ...
                if self.data[self.pos] == '<':
                    ...
                    if t == '/':
                        ...
                        '<?'
                    ):   # FIXME: use t and modify self.pos?
            ...
            if self.now_at('&#'):
                ...
                try:
                    ...
                    if ent.is_internal():
                        ...
                        self.push_entity(
                            self.get_current_sysid(), ent.value, name
                        )
    ...
    def parse_ent_ref(self):
        ...
        if ent.is_internal():
            ...
            self.open_entity(
                self.pubres.resolve_entity_pubid(
                    ent.get_pubid(), ent.get_sysid()
                ),
                name,
            )
            self.seen_root = (
                1  # Entity references only allowed inside elements
            )
    ...
    def parse_doctype(self):
        ...
        if (
            pub_id != None or sys_id != None
        ):   # Was there an external id at all?
            if self.read_external_subset:
                try:
                    ...
                    p.parse_resource(
                        join_sysids(self.get_current_sysid(), sys_id)
                    )
                finally:
                    p.deref()
        ...
        if (
            pub_id == None and sys_id == None
        ) or not self.read_external_subset:
